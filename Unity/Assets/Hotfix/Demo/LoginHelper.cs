...
namespace ET
{
    public static class LoginHelper
    {
        ...
        {
            Log.Warning(zoneScene.Name + " " + zoneScene.Id);
        ...
        public static void AutoLogin(Scene zoneScene)
        {
            ...
            //获取当前登陆数据
            LoginStateComponent loginState = zoneScene.GetComponent<LoginStateComponent>();
            if (loginState.IsAutoConnect)
        ...
        public static async ETVoid BackLogin(Scene zoneScene)
        {
            ...
            //TODO:切换到登陆场景
            await Game.EventSystem.Publish(new EventType.ChangeScene() { ZoneScene = zoneScene, SceneType = "Init" });
        }
        #endregion
        ...
        private static async ETTask<R2C_Login> ConnectRealm(Scene zoneScene, string address, string account)
        {
            try
            {
                ...
                {
                    session.GetComponent<SessionCallbackComponent>().DisposeCallback = RealmSessionDispose;
                    r2cLogin = await session.Call<R2C_Login>(new C2R_Login() { Account = account, Password = "111111" });
                }
                if (r2cLogin == null)
                    throw new Exception($"登陆验证服失败 Error:r2cLogin is null");
                if (!string.IsNullOrEmpty(r2cLogin.Message))
                    throw new Exception($"登陆验证服失败 Error:{r2cLogin.Message}");
                Log.Debug($"连接验证服成功");
                return r2cLogin;
            }
            catch (Exception e)
        ...
        private static async ETTask<G2C_LoginGate> ConnectGate(Scene zoneScene, R2C_Login r2CLogin)
        {
            try
            {
                ...
                G2C_LoginGate g2cLoginGate = await gateSession.Call<G2C_LoginGate>(new C2G_LoginGate() { Key = r2CLogin.Key, GateId = r2CLogin.GateId});

                if (g2cLoginGate == null)
                    throw new Exception($"登陆网关服失败 Error:g2cLoginGate is null");
                if (!string.IsNullOrEmpty(g2cLoginGate.Message))
                    throw new Exception($"登陆网关服失败 Error:{g2cLoginGate.Message}");
                Log.Debug($"连接网关服成功");
                return g2cLoginGate;
            }
            catch (Exception e)
        ...
        private static void RealmSessionDispose(Session session)
        {
            ...
            {
                //TODO:打开Tips提示错误信息
                Game.EventSystem.Publish(new EventType.ShowTips() { ZoneScene = session.ZoneScene(), Tips = tips });
            }
        }
        /// <summary>
        /// 网关服session销毁
        ...
        {
            Log.Debug("开始登陆");
            zoneScene.GetComponent<LoginStateComponent>().NetworkState = E_NetworkState.BebeingConnect;
            //打开菊花
        ...
        private static void ResetConnect(Scene zoneScene)
        {
            Log.Debug("断开链接并重置状态");
            //状态改为断开连接
            zoneScene.GetComponent<LoginStateComponent>().NetworkState = E_NetworkState.Disconnect;
        ...
        {
            Log.Debug($"重连成功 Count:{zoneScene.GetComponent<LoginStateComponent>().ReconnectionCount}");
            // BaseModel.SetSelf(g2CLoginGate.User);
        }
        /// <summary>
        /// 重连失败
        ...
        private static async void ReconnectFailure(Scene zoneScene)
        {
            ...
            //断线自动重连5次，5次后提示是否重连
            if (loginState.ReconnectionCount < 5)
            {
        ...
        {
            Game.EventSystem.Publish(new EventType.OpenMessageBox() 
            {
                ZoneScene = zoneScene, 
                Tips = tips, 
