...
namespace ET
{
    public class RecastPathAwakeSystem: AwakeSystem<RecastPathComponent, int>
    {
        public override void Awake(RecastPathComponent self, int mapId)
        {
            self.Awake(mapId);
        }
    }
    ...
    public class RecastPathComponent: Entity
    {
        ...
        /// 5v5地图的Nav数据路径
        /// </summary>
        public const string MapNavDataPath = "../Config/RecastNavData/";
        ...
        /// 初始化寻路引擎
        /// </summary>
        public void Awake(int mapId)
        {
            RecastInterface.Init();
            //TODO 先直接在这里强行初始化地图
            LoadMapNavData(mapId, ($"{MapNavDataPath}Map_Navmesh_{mapId}.bin").ToCharArray());
            // // 读取体素数据
        ...
        public void LoadMapNavData(int mapId, char[] navDataPath)
        {
            ...
            {
                Log.Error($"加载Id为{mapId}的地图Nav数据失败！");
            }
        }
